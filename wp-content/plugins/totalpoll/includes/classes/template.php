...
if ( ! class_exists( 'TP_Template' ) ) :
	...
	abstract class TP_Template {
		...
		public function style() {
			...
			if ( is_readable( $cached ) && ( defined( 'DOING_AJAX' ) === false || DOING_AJAX === false ) ):
		 ...
		 * @since 3.0.0
		 */
		public function render( $fragment = 'vote', $css = true, $extra_attributes = '' ) {
			if ( ! empty( $fragment ) ):
			...
			return sprintf(
				'%1$s<div id="%2$s" class="%3$s"%4$s>%5$s</div>',
				$css === true ? $this->style() : '',
		...
		public function buttons() {
			...
			if ( $this->current === 'vote' ):
				...
				if ( $this->poll->settings( 'results', 'hide', 'enabled' ) === false && $this->poll->settings( 'limitations', 'results', 'require_vote', 'enabled' ) === false ):
